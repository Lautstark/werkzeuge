...
**`el()` and `byId()`, since 2.0.0.** `./dom` shipped a builder and a finder for
the years the four products drew with `document.createElement` — one name that
meant *make* in two of them and *fetch* in a third was the reason it was one
module. All four draw with Svelte components since 2026-09-17 and a component
holds its own nodes, so the last two call sites went with the markup they
looked into, and the module went with them. `$` was never offered and still is
not: it reads well inside one file and is a name nobody can search for in a
package.
